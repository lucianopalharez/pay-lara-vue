  ...
  <div>
    ...
    <div v-if="($page.props.flash.error || Object.keys($page.props.errors).length > 0) && show" class="flex items-center justify-between mb-8 max-w-3xl bg-red-500 rounded">
      <div class="flex items-center">
        ...
        <div v-if="Object.keys($page.props.errors).length > 1" class="py-4 text-white text-sm font-medium">
          <span>Ops, verifique os erros</span>
        </div>
        <div v-else="$page.props.flash.error" class="py-4 text-white text-sm font-medium">{{ $page.props.flash.error }}</div>
